...
export const SYSTEM_EXPLAIN = [
  ...
  "- At most 3 short sentences. Mention the most important number from the FACTS. No lists, no headings.",
...
// "help" comes first: small models lean towards the last example they saw.
const EXAMPLES = [
  ...
  ["is 48 possible for me", "solve_for"],
  ["what's eating up the money", "corpus"],
];

export function classifyMessages(question) {
  return [
    { role: "system", content: `Classify the question about a FIRE plan into exactly one label. Reply with the label only. Use help only for greetings or questions about this tool.\nLabels:\n${LABELS.map((l) => `${l}: ${INTENTS[l]}`).join("\n")}` },
